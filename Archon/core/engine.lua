...
function Engine:start()
    ...
    for name in pairs(self.di.factories) do
    ...
    for _, name in ipairs(self.di.stack) do
...
function Engine:stop()
    ...
    for i = #self.di.stack, 1, -1 do
        ...
        if inst and type(inst.Destroy) == "function" then
            local ok, err = pcall(inst.Destroy, inst)
            if not ok then warn("[Engine] Destroy failed " .. name .. ": " .. tostring(err)) end
        end
    end
