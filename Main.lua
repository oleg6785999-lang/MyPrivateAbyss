-- =============================================
-- ABYSS ARCHON v1005.420 — MODULAR PROFESSIONAL EDITION
-- =============================================
...
local hitboxCache = {}
local connections = {}
local lastAuraTime, lastRapidTime, lastJumpTime = 0, 0, 0
local AntiDetected = false
local lastAntiCheck, lastTriggerTime = 0, 0
...
local function AimbotFunction()
    while task.wait() do
        if not Settings.Aimbot.Enabled or not isAiming or not cachedRoot then continue end
        local closest = nil
...
local function ESPFunction()
    while task.wait(0.016) do
        if not Settings.ESP.Enabled or not drawingAvailable then continue end
        for _, plr in ipairs(allPlayers) do
...
local function SpeedHackFunction()
    while task.wait() do
        if Settings.SpeedHack.Enabled and cachedCharacter then
            local hum = cachedCharacter:FindFirstChild("Humanoid")
            if hum then hum.WalkSpeed = Settings.SpeedHack.Speed end
        end
    end
end
...
local function InfJumpFunction()
    while task.wait() do
        if Settings.InfJump and cachedCharacter then
            local hum = cachedCharacter:FindFirstChild("Humanoid")
...
local function TriggerbotFunction()
    while task.wait() do
        if Settings.Triggerbot and isAiming and mouseClickExists and tick() - lastTriggerTime > Settings.TriggerDelay then
            ...
            for _, plr in ipairs(allPlayers) do
                if IsEnemy(plr) and plr.Character then
                    ...
                    if head then
                        ...
                        if on and (Vector2.new(sp.X, sp.Y) - mouse).Magnitude < 40 then
                            mouse1click()
...
local function AuraFunction()
    while task.wait() do
        if Settings.Aura.Enabled and cachedRoot and tick() - lastAuraTime > Settings.Aura.Delay then
            lastAuraTime = tick()
            for _, plr in ipairs(allPlayers) do
                if IsEnemy(plr) and plr.Character then
                    local root = plr.Character:FindFirstChild("HumanoidRootPart")
                    local hum = plr.Character:FindFirstChild("Humanoid")
                    if root and hum and (root.Position - cachedRoot.Position).Magnitude <= Settings.Aura.Radius then
                        hum:TakeDamage(Settings.Aura.Damage)
                    end
                end
            end
        end
...
local function RapidFireFunction()
    while task.wait() do
        if Settings.RapidFire and tick() - lastRapidTime > 0.05 then
            lastRapidTime = tick()
            local tool = cachedCharacter and cachedCharacter:FindFirstChildOfClass("Tool")
...
if Rayfield then
    local Window = Rayfield:CreateWindow({
        Name = "ABYSS ARCHON • MODULAR 2026",
    ...
    Tab_Combat:CreateToggle({Name = "Silent Aim", CurrentValue = false, Callback = function(v) Settings.Aimbot.Silent = v end})
    ...
    Tab_Movement:CreateSection("Movement")
    ...
    Tab_Movement:CreateToggle({Name = "SpeedHack", CurrentValue = false, Callback = function(v) Settings.SpeedHack.Enabled = v end})

    Rayfield:LoadConfiguration()
    Rayfield:Notify({Title = "ABYSS ARCHON", Content = "v1005.420 MODULAR EDITION | FULLY PROFESSIONAL | ГОТОВ К АННИГИЛЯЦИИ", Duration = 8})
end

print("ABYSS ARCHON v1005.420 — MODULAR PROFESSIONAL EDITION AWAKENED")

-- ==================== LAUNCH FUNCTIONS ====================
task.spawn(AimbotFunction)
task.spawn(ESPFunction)
task.spawn(FlyFunction)
task.spawn(AntiAimFunction)
task.spawn(SpeedHackFunction)
task.spawn(InfJumpFunction)
task.spawn(TriggerbotFunction)
task.spawn(AuraFunction)
task.spawn(RapidFireFunction)
